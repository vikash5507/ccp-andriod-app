...
import 'package:ccp/widgets/createPost/ComposeTweetImage.dart';
import 'package:ccp/widgets/createPost/composeBottomIconWidget.dart';
import 'package:ccp/widgets/createPost/widgetView.dart';
...
class CreatePostPage extends StatefulWidget {
  CreatePostPage({Key key, this.isRetweet, this.isTweet = true})
      : super(key: key);
...
class _CreatePostReplyPageState extends State<CreatePostPage> {
  ...
  void _submitButton() async {
    ...
    await Provider.of<CreatePostState>(context, listen: false)
        .sendNotification(
  ...
  FeedModel createTweetModel() {
    ...
    print("MyUser: ");
    print(myUser);
    var profilePic = dummyProfilePic;//myUser.profilePic ?? 
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        isCrossButton: true,
        submitButtonText:
            widget.isTweet ? getTranslated(context, 'create_post_page_button') : widget.isRetweet ? 'RePost' : 'Reply',
        isSubmitDisable:
            !Provider.of<CreatePostState>(context).enableSubmitButton ||
                Provider.of<FeedState>(context).isBusy,
        isbootomLine: Provider.of<CreatePostState>(context).isScrollingDown,
      ),
...
class _ComposeRetweet
    extends WidgetView<CreatePostState, _CreatePostReplyPageState> {
            ...
            children: <Widget>[
              Padding(
                ...
                child:
                    ...
                    customImage(context, dummyProfilePic, height: 40),
              ),
              Expanded(
                child: _TextField(
                  isTweet: false,
...
class _ComposeTweet
    extends WidgetView<CreatePostPage, _CreatePostReplyPageState> {
        ...
        children: <Widget>[
          ...
          Row(
            ...
            children: <Widget>[
              ...
              customTitleText(authState.user.fullname),
              
            ],
          ),
          Divider(),//ToDO - separate comment UI here (changes made for create fresh post)
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Expanded(
      ...
      children: <Widget>[
        TextField(
          ...
          decoration: InputDecoration(
              ...
              hintText: isTweet
                  ? getTranslated(context, 'create_post_page-body_text')
...
class _UserList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return !Provider.of<CreatePostState>(context).displayUserList ||
            list == null ||
            list.length < 0 ||
            list.length == 0
        ? SizedBox.shrink()
                ...
                return _UserTile(
                  ...
                  onUserSelected: (user) {
                    textEditingController.text =
                        Provider.of<CreatePostState>(context)
                            .getDescription(user.userName);
                    textEditingController.selection = TextSelection.collapsed(
                        offset: textEditingController.text.length);
                    Provider.of<CreatePostState>(context).onUserSelected();
                  },
                );
              },
            ),
